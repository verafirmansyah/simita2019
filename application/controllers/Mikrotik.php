...
class Mikrotik extends CI_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->load->model('m_mikrotik');
        // load helper Date
        $this->load->helper('date');
        chek_session();
    }
	public function index() {
        $data['record'] = $this->m_mikrotik->semua()->result(); 
        $this->template->display('mikrotik/view', $data);       
    }		
    ...
    function add() {                
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                'kode_mikrotik' => $this->m_mikrotik->kdotomatis(),
            ...
            $this->m_mikrotik->simpan($data);
            redirect('mikrotik');
        } else {           
        	$data['supplier'] = $this->m_mikrotik->getsupplier()->result(); 
            $data['cabang'] = $this->m_mikrotik->getcabang()->result();
            $this->template->display('mikrotik/tambah',$data);
        }
    }
    ...
    function edit() {       
        if (isset($_POST['submit'])) {
            ...
            if ($this->form_validation->run() == true) {
                $data = array(                            
                    'kode_lisensi' => $this->m_mikrotik->kdotomatis(),
                ...
                $this->m_mikrotik->edit($kode,$data);
                ...
                $data['record'] = $this->m_mikrotik->getkode($id)->row_array();
                $this->template->display('lisensi/edit', $data);
            }
    }
    function delete($id) {
        if ($this->session->userdata('role')=='Administrator'){
            $this->m_mikrotik->hapus($id); 
            redirect('lisensi'); 
        }else{
    ...
    function detail() { 
        $id = $this->uri->segment(3);                                           
        $data['recordall'] = $this->m_mikrotik->getall($id)->row_array();
        $this->template->display('lisensi/detail', $data);            
    }

    function view_inventaris() { 
        $id = $this->uri->segment(3);                                           
        $data['recordall'] = $this->m_mikrotik->view_inventaris($id)->row_array();
    ...
    $lisensi = $this->m_mikrotik->semua()->result();
